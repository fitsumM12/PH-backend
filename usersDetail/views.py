...
from .models import DashboardProfile
from .serializers import LandingPageImageSerializer

class LandingPageImageView(APIView):
    permission_classes = [permissions.AllowAny]

    def get(self, request, *args, **kwargs):
        try:
            profile = DashboardProfile.objects.latest('updated_at')
            serializer = LandingPageImageSerializer(profile, context={'request': request})  
            return Response(serializer.data)
        except DashboardProfile.DoesNotExist:

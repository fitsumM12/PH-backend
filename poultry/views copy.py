    ...
    for chicken in chickens:
        ...
        for record in total_deaths:
            death_data.append({
                ...
                "year": record['year'],
                "month": record['month'],
                "total_deaths": record['total_deaths'] or 0  
            })
...
def fetch_individual_egg_dashboard_api(request):
    try:
        top_eggs = IndividualEggProduction.objects.select_related(
            'bird', 'bird__breed'
        ).order_by('-egg_count')[:3]
        serializer = IndividualEggProductionDashboardSerializer(top_eggs, many=True)
